...
void company::Date::print_date() const {
    std::cout << month_ << "/" << day_ << "/" << year_ << '\n';
}

bool company::Date::is_leap_year(ushort year) {
    if (year % 4 == 0) {
        if (year % 100 == 0) {
...
ushort company::Date::get_days_in_month(ushort month, ushort year) const {
    switch (month) {
        case 2:
...
bool company::Date::is_valid_date(ushort day, ushort month, ushort year) const {
